...
namespace Localization
{
        ...
        private void LoadForCulture(CultureInfo culture)
        {
            ...
            foreach (var c in candidates.Distinct())
            {
                try
                {
                    ...
                    //Trace.WriteLine($"[Localization] Loaded resource candidate: {c}");
                }
                catch (Exception ex)
                {
                    _logging?.Error(ex, $"Failed to load '{c}' -> {ex.GetType().Name}");
                    //Trace.WriteLine($"[Localization] failed to load '{c}' -> {ex.GetType().Name}: {ex.Message}");
                    // not found -> skip
                }
            }
            ...
            else
            {
                ...
                //Trace.WriteLine("[Localization] Application.Current is null -> skipping App.Resources merge (design-time).");
            }
        }
    }
    ...
    public class LocExtension : MarkupExtension
    {
        ...
        {
            //Trace.WriteLine($"[LocExtension] ProvideValue for key='{Key}', ProviderInitialized={LocalizationProvider.Instance.IsInitialized}");
            ...
            if (!LocalizationProvider.Instance.IsInitialized && DesignerHelper.IsInDesignTool())
            {
                    ...
                    if (rootProvider?.RootObject is FrameworkElement root)
                    {
                        ...
                        if (xmlLang != null && !string.IsNullOrEmpty(xmlLang.IetfLanguageTag))
                        {
                            try
                            {
                                ...
                                Trace.WriteLine($"[LocExtension] Design language detected: {xmlLang.IetfLanguageTag}");
                            }
                            catch (Exception ex)
                            {
                                Trace.WriteLine($"[LocExtension] invalid design language '{xmlLang.IetfLanguageTag}': {ex.Message}");
                            }
                        }
                                ...
                                if (!(root.Tag is string t && t.Contains(tokenKey)))
                                {
                                    EventHandler handler = (_, __) =>
                                    {
                                        try
                                        {
                                            ...
                                            {
                                                Trace.WriteLine($"[LocExtension] Design language changed -> {newLang.IetfLanguageTag}");
                                        ...
                                        {
                                            Trace.WriteLine($"[LocExtension] error handling design language change: {ex}");
                                        }
                                    };
                        ...
                        {
                            Trace.WriteLine($"[LocExtension] failed to attach language change listener: {ex}");
                        }
                    }
                ...
                {
                    Trace.WriteLine($"[LocExtension] design init failed: {ex}");
                }
            }
